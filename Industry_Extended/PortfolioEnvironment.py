...
class LoaderEnginering:
    ...
    # Always concatenate and calculate continuously for consistency
        ...
        extended_dates = self.data_exp_extend['date'].unique()
        ...
        self.tic_date_extend = roc_combined  # full version
        
        # Return original references for backward compatibility
        data = self.data.reset_index(drop=True)
        data_extend = self.data_exp_extend.reset_index(drop=True)
    ...
    def data_split(self, df, train_size, val_size, feature_list, extended_data=None):
        ...
        # Concatenate extended data to test BEFORE scaling
        is_extended = False
        if extended_data is not None:
            test_last_date = test_data['date'].max()
            extended_after = extended_data[extended_data['date'] > test_last_date] # use the extension
    ...
    def prepare_train_test_df(self):
        ...
        # Process both original and extended data
        data_with_features, data_extend_with_features = self._add_features_technical()
        
        # Track original data start (information starting point)
        ...
        # Apply cov to both (this consumes first 64 days)

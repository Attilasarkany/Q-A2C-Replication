...
class QACAgent:
    ...
    '''
    transaction cost + number of stocks (weights)+ wealth + lower triangular (n*(n+1)/2) +VIX+ roc + maxdrawdown
    1 + 5 + 1+ 15 +1
    1+10+1 +55
    1+10+1+55+1+10+1
    1 + 25 + 1+325+ 1+25+1 379
    '''
        
    @tf.function(
        input_signature=[
            tf.TensorSpec(shape=(None, 34), dtype=tf.float32),  # in case of 3 stocks:12
            tf.TensorSpec(shape=(None, 34), dtype=tf.float32),  # in case of 3 stocks:12
            tf.TensorSpec(shape=(None,10), dtype=tf.float32),   # in case of 3 stocks: 3
